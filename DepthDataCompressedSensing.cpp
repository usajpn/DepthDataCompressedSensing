...
#include "DepthDataCompressedSensing.h"
...
std::vector<std::string> kl1p::Split(std::string &src, std::string key) {
	std::string str = src;
	std::vector<std::string> str_line;
	...
	while (index < str_len) {
		...
		index = (index == std::string::npos ? str_len : index);
		std::string tmp = str.substr(oldindex, index - oldindex);
...
void	kl1p::RunExample()
{
	try
	{
		...
		if (bWrite)
			kl1p::WriteToCSVFile(y, "CompressedSignal.csv");
